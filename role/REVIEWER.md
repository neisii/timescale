...
9. **Docker 즉시 실행 가능성 (클론 후 `docker-compose up` 한 명령으로 동작해야 함)**

   **Kafka 설정**
   - `KAFKA_ADVERTISED_LISTENERS`가 내부(`kafka:9092`)와 외부(`localhost:29092`) 리스너로 분리되어 있는가?
   - `KAFKA_LISTENER_SECURITY_PROTOCOL_MAP`이 두 리스너를 모두 포함하는가?
   - Producer·Consumer 서비스가 컨테이너 내부 주소(`kafka:9092`)로 접속하는가?
   - Kafka 토픽이 init 컨테이너 또는 스크립트로 자동 생성되는가?

   **서비스 기동 순서**
   - Kafka, TimescaleDB, Redis 각각에 `healthcheck`가 정의되어 있는가?
   - Producer·Consumer·API 서비스가 `condition: service_healthy`로 인프라 준비를 대기하는가?
   - 애플리케이션 서비스에 `restart: on-failure`가 설정되어 있는가?

   **TimescaleDB 초기화**
   - `timescale/timescaledb` 이미지를 사용하는가? (`postgres` 이미지 사용은 High 이슈)
   - `/docker-entrypoint-initdb.d/`에 마운트된 init SQL 스크립트가 존재하는가?
   - init 스크립트에 `CREATE EXTENSION timescaledb`와 `create_hypertable()` 호출이 포함되어 있는가?

   **환경변수 및 설정**
   - `.env.example` 파일이 존재하고 모든 필수 환경변수가 기본값과 함께 정의되어 있는가?
   - `.env`가 `.gitignore`에 등록되어 있는가?

   **검증 스크립트**
   - `infra/verify.sh`가 존재하고 전체 스택 상태를 자동 점검하는가?

   > 항목 9의 모든 세부 항목은 하나라도 실패하면 **Severity: High / Category: A 또는 B**로 분류한다.
...
- **체크리스트 항목을 생략하지 않는다.** 9개 항목을 반드시 모두 검토한다.
...
[Checklist Summary]
- 9개 항목별 통과(PASS) / 실패(FAIL) / 부분(PARTIAL) 결과
```

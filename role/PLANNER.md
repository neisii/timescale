...
### plan.md 작성
설계 완료 후 루트 경로에 `plan.md`를 생성한다. 이 파일은 다음 목적을 동시에 충족한다:
- **세션 간 연결**: 새 세션을 시작하는 모든 Agent가 "참고해서 진행해줘"로 바로 맥락을 복원할 수 있는 단일 진입점
- **포트폴리오 진입점**: 레포를 처음 보는 사람이 30초 안에 전체 설계를 파악할 수 있도록 구성
